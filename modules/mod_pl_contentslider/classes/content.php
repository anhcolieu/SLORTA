...
class BtContentDataSource extends PLSource {
        ...
        foreach ($items as &$item) {
            ...
            if ($showimage) {
				if($item->images && $use_introimg){
					...
					if(!$imgLink){
						$imgLink = $item->images->image_fulltext;
					}
					if($imgLink){
						$item->mainImage = $item->images->image_intro;
						if ($isThumb){
							$item->thumbnail = self::renderThumb($item->images->image_intro, $thumbWidth, $thumbHeight, $isThumb);
						}
						else {
							$item->thumbnail = $item->images->image_intro;
						}
					}
				}
				if(!$item->thumbnail){
