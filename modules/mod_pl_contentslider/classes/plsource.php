...
if(!class_exists('PLSource')){
abstract class PLSource{
		...
		if ($isThumb){
			...
			if (file_exists($imagSource)) {	
				if (!file_exists($thumbPath)) {
					//create thumb
					PLImageHelper::createImage($imagSource, $thumbPath, $width, $height, true);
				}
				return $this->_thumbnaiURL . $imageName;
				 ...
				 if (!file_exists($thumbPath)){
					 ...
					 // Image loaded?
					 if ($this->_CreateImageUsingCurl( $path, $thumbPath, 5 )) {
						 PLImageHelper::createImage($thumbPath, $thumbPath, $width, $height, true);
						 return $this->_thumbnaiURL . $imageName;
					 }
				 } else {
					 return $this->_thumbnaiURL . $imageName;
	...
	/**
	 * Get a suplring with the max length setting.
	 *
	 * @param string $text;
	...
	/**
	 * Get a suplring with the max word setting
	 *
	 * @param string $text;
	 * @param int $length limit characters showing;
	 * @param string $replacer;
